...
def handle_missing_values(df, missing_threshold=0.8):
    ...
    # Drop high-missing columns (deterministic: same threshold → same columns)
    cols_to_keep = sorted(miss[miss < missing_threshold].index.tolist())
    ...
    # KNN imputation - DETERMINISTIC because:
    # 1. Input order is fixed (sorted stay_id from standardize)
    # 2. Chunk boundaries are fixed (same chunk_size)
    # 3. KNNImputer with n_neighbors=1 picks nearest by Euclidean distance
    # 4. Ties broken by lowest index in the chunk (sklearn implementation)
...
def apply_exclusion_criteria(df):
    """Memory-efficient exclusion criteria - no Polars conversion."""
...
def main():
    ...
    # Save - DETERMINISTIC: sorted output
