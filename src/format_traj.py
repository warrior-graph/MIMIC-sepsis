...
def handle_missing_values(df, missing_threshold=0.8):
    ...
    # Drop columns with too much missing — but NEVER drop score-critical columns
    cols_to_keep = [
    ...
    # KNN imputation for remaining (includes protected cols that exceed normal threshold)
...
def calculate_derived_variables(df):
    ...
    # Supplemental oxygen: fio2 > 21% means on supplemental O2 → +2
    if 'fio2' in df.columns:
...
def main():
    ...
    def _process_cohort(cohort_df, label):
    ...
    onset_data = _process_cohort(onset, 'onset')

    # Process non-onset (control) patients through identical pipeline
    non_onset_data = _process_cohort(non_onset, 'non-onset')

    # Combine all patient data — onset first, then controls
    init_traj = pd.concat(onset_data + non_onset_data, ignore_index=True)
    print(f'  Combined cohort: {init_traj["stay_id"].nunique()} patients, {len(init_traj)} rows')
    ...
    # Apply exclusion criteria (multi-score gate + noise injection)
